...
namespace IBM.Watson.Examples
{
    public class ExampleTextToSpeechV1 : MonoBehaviour
    {
        ...
        private string synthesizeText = "何かしゃべってください";
        ...
        public void TextToSpeech(string result)
        {
            if (result == "good")
            {
                Runnable.Run(ExampleSynthesize("↑すごいね！"));
            }
            else if (result=="normal")
            {
                Runnable.Run(ExampleSynthesize("↑なるほど!↑"));
            }
            else if (result=="bad")
            {
                Runnable.Run(ExampleSynthesize("悪いのは君じゃない↓"));
            }
            else
            {
